...
`cost.source` is `"provider"` when the total came from the provider, including OpenRouter streamed or reconciled costs, and `"pi"` when pi calculated it from model pricing metadata.
